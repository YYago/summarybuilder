...
* 用[node-modules-custom](https://www.npmjs.com/package/node-modules-custom) 替换原有批量创建文件函数。
* .SBer_summary( jsonFileName, isIndent, outFileName ) —— option： `jsonFileName` can be an Array. 可以直接用数组或者类型为数组的Markdown 文件列表，这样就不必麻烦生成 .JSON 文件了。
* 更改console.log() 的内容并使用[chalk](https://www.npmjs.com/package/chalk)进行着色，避免和其他package的consol.log()内容混在一起。
...
特意新增：`SBer_summary()` 和 `SBer_createMD()` 供使用。
